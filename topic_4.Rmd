...
```{r}
tweets_new <- tweets # duplicate the original tweets df 
...
words_summary %>% 
  ...
  geom_col(aes(fill = as.factor(date)))
...
```{r}
# do a bar graph of counts for positive, negative, and neutral twice to compare?

# sentiment(raw_tweets$Title)
